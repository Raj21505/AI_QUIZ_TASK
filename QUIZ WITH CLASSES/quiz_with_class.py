class Admin():
            ...
            try:
                ...
                if update_list <= 0 or update_list > len(Quetions):
                    ...
                    while True:
                        ...
                        if user == "QUE":
                            Admin.update_que(update_list)

                        elif user in ["A","B","C","D"]:
                            Admin.update_opt(update_list ,user)

                        elif user == "ANS":
                            Admin.update_ans(update_list)
    ...
    def update_opt(update_list,user):
        ...
        Quetions[update_list][user_update] =  update_option
        print("\n Updated the Option \n")
...
class Further:
    def further(a):
        while True:
            print(f"Do you want to {a} further..?  (Enter 'No' to Quit)")
            leave = input("Enter :- ")
            print()
            if leave.upper() == "NO":
                print("Okay... Question's list got updated")
                break
            elif a == "add":
                return Admin.add()
            elif a == "remove":
...
while True:
    try:
        ...
        if role == 1:
            ...
            print("\n --> Enter valid Number ( 1 or 2 or 3) <-- \n")
